...
public:
	...
	void update(int pos, int val) {
		if(!pos) return;
		for(; pos<bit.size(); pos+=pos&(-pos)) bit[pos]+=val;
	}
...
Bit bit;
int n, q, p[N], h[N], lca[N][M], answer[N], sz[N];
string s;
vector<int> adj[N];
...
void add(char c, int pos) {
	p[pos] = p[pos-1];
	while(p[pos] && s[p[pos]] != c) p[pos] = p[p[pos]-1];
	p[pos]+= c == s[p[pos]];

	int u = pos+1;
	lca[u][0] = p[pos]; //consider indexes 1-indexed
	h[u] = h[p[pos]]+1;
	for(int i=1; i<M; i++) {
		lca[u][i] = lca[lca[u][i-1]][i-1];
	}
	adj[lca[u][0]].push_back(u);//adds edge of tree
}
